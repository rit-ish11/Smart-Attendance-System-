...
REPORT_CSV = os.path.join(BASE_DIR, f"attendance_report_{timestamp_now}.csv")
...
# ---------- Open Camera for One Snapshot ----------
cap = cv2.VideoCapture(0)
print("[INFO] Press SPACE to capture photo, Q to quit.")
...
if captured_frame is not None:
    ...
    for encoding, box in zip(encodings, boxes):
        ...
        # Draw bounding box + name/roll_no on top
        (top, right, bottom, left) = box
        cv2.rectangle(captured_frame, (left, top), (right, bottom), color, 2)
        cv2.putText(captured_frame, f"{name} ({roll_no})", (left, top - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

    # Show final recognized image
    cv2.imshow("Recognized Snapshot", captured_frame)
    cv2.waitKey(3000)  # show for 3 sec
    cv2.destroyAllWindows()

# ✅ Export attendance to unique CSV with Present/Absent
